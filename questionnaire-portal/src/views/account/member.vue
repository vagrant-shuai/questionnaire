        ...
        <div v-if="userInfo" class="account-info-view">
          <div style="margin-left: 30px; display: flex; flex-direction: row">
            <div class="account-icon-view">
              ...
              <el-button v-else type="text" @click="bindWxHandle"
                >绑定</el-button
              >
              <el-dialog
                title="微信扫描二维码绑定"
...
export default {
  ...
  methods: {
    ...
    sendUpdatePhoneNumber() {
      this.$refs["updatePhoneForm"].validateField("phoneNumber", (err) => {
        if (!err) {
          this.$api
            .get(
              `/user/update-phone/code?phoneNumber=${this.userInfoForm.phoneNumber}`
            )
            .then(() => {
    ...
    updateUserPhoneHandle() {
      this.$refs["updatePhoneForm"].validateField(
        ["phoneNumber", "code"],
        (err) => {
          if (!err) {
            this.$api
              .post("/user/update/phone-number", this.userInfoForm)
              .then(() => {
                this.msgSuccess("修改成功");
                this.queryUserInfo();
              });
