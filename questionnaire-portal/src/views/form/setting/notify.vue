    ...
    querySubNotifyWxUser(openIdStr) {
      getWxNotifyUserRequest({
        ...
        if (this.subNotifyWxUserList) {
          let changeNewWriteNotifyWx = this.subNotifyWxUserList
            .map((item) => item.openId)
            .join(";");
          if (
            !openIdStr &&
            changeNewWriteNotifyWx != this.notifySettingForm.newWriteNotifyWx
          ) {
    ...
    deleteSubNotifyUserHandle(i) {
      if (this.subNotifyWxUserList) {
        ...
        deleteWxNotifyUserRequest(key, openId).then((res) => {
          ...
          this.notifySettingForm.newWriteNotifyWx = this.subNotifyWxUserList
            .map((item) => item.openId)
            .join(";");
        });
      }
