...
export default {
  ...
  methods: {
    ...
    queryProjectResult() {
      this.$api
        .get("/user/form/result/public/page", { params: this.queryConditions })
        .then((res) => {
    ...
    queryProjectItems() {
      this.$api
        .get("/user/form/item/list", {
          params: { key: this.projectKey, displayType: true },
        })
        .then((res) => {
          if (res.data) {
            res.data.map((item) => {
              _.set(
                this.projectItemColumns,
                `field${item.formItemId}`,
                item.label
              );
            });
          }
