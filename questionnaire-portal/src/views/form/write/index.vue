<template>
  <div class="write-container">
    ...
    <div v-cloak v-if="writeStatus == 2" class="title-icon-view">
      <div
        v-if="userFormSetting.submitShowType === 2"
        v-html="userFormSetting.submitShowCustomPageContent"
      />
...
import {
  checkWriteCountCap,
  saveWriteCount,
} from "@/views/form/write/DeviceWriteCount";
  ...
  methods: {
    ...
    checkDeviceWriteLimit() {
      if (this.userFormSetting.deviceWriteCountLimitStatus) {
        const flag = checkWriteCountCap(
          this.formKey,
          this.userFormSetting.deviceWriteCountLimit
        );
        if (flag) {
          this.writeNotStartPrompt =
            this.userFormSetting.deviceWriteCountLimitText;
          this.writeStatus = 0;
        }
        return flag;
      }
    ...
    async submitForm(data) {
      ...
      this.submitFormData.formType =
        this.$refs.bizProjectForm.formConf.formType;
      let res = null;
      if (this.writeType === 1) {
