<template>
  ...
  >
        ...
        <el-descriptions :column="1" title="基础信息">
          ...
          >
            ...
            <span v-if="field.value === 'wxUserInfo'">
              ...
              <div>
                {{ formModel.wxUserInfo ? formModel.wxUserInfo.openid : "" }}
              </div>
            </span>
            <span v-else>
    ...
    <span slot="footer" class="t-dialog__footer">
      <el-button v-if="isEditMode" type="primary" @click="handleUpdate">
        确 定
      </el-button>
      <el-button
        ...
        this.formConf.fields = val
          ...
          .map((item) => {
            if (item.scheme) {
              ...
              // 去除标签中富文本
              item.scheme.config.label = removeHtmlTag(
                item.scheme.config.label
              );
            }
            return item.scheme;
          });
