  ...
  <el-form ref="form" label-width="80px" label-position="top" class="view-form">
    <div v-for="field in tempFields" :key="field.value">
      <el-form-item class="ml15">
        ...
        <div
          v-else-if="
            field.type === 'USER_SELECT' || field.type === 'DEPT_SELECT'
          "
        >
          <div v-if="pageFormModel[field.value]">
        ...
        <div v-else-if="field.type === 'GOODS_SELECT'">
          <div v-if="pageFormModel[field.value]">
            <text-copy
              :text="formatGoodsValue(pageFormModel[field.value], field.scheme)"
            />
          </div>
        </div>
        <!--   预约-->
        <div v-else-if="field.type === 'RESERVE_DAY'">
          <div v-if="pageFormModel[field.value]">
            <text-copy
              :text="
                formatReserveValue(pageFormModel[field.value], field.scheme)
              "
            />
          </div>
        </div>
        <div v-else-if="field.type === 'RESERVE_TIME_RANGE'">
          <div v-if="pageFormModel[field.value]">
            <text-copy
              :text="
                formatReserveTimeValue(pageFormModel[field.value], field.scheme)
              "
            />
          </div>
        </div>
        ...
        <div v-else-if="field.type === 'PROVINCE_CITY'">
          <div v-if="pageFormModel[field.value]">
            <text-copy
              :text="
                formatProvinceValue(pageFormModel[field.value], field.scheme)
              "
            />
          </div>
        </div>
